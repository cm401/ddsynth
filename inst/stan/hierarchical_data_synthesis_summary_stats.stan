...
functions {
  ...
  real dc_log_lik(real ex_l, real ex_r, real ev_l, real ev_r,
    ...
    // Case 1: both endpoints point-observed. The implied delay ev_l - ex_l is
    // itself a rounded reported value (like summary_type 4/5's degenerate
    // case), not an exact continuous observation, and is treated the same
    // way via rounded_freq_loglik_fun rather than the raw point density
    // dist_logpdf_fun (which is singular/undefined at a delay of exactly 0
