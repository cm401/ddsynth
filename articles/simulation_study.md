...
``` r
# Resolve path: installed package uses inst/extdata; source tree falls back to
# the same file relative to the package root discovered by here().
rds_path <- system.file("extdata", "simulation_results_all.rds", package = "ddsynth")
if (!nzchar(rds_path)) {
  rds_path <- file.path(here(), "inst", "extdata", "simulation_results_all.rds")
}
